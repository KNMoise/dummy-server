<?php
	...
	if (isset($_POST['update'])) {
		...
		if (empty($testimonial_name) || empty($testimonial_company) || empty($testimony)) {
			header("location: ../client_side_pages.php?error=notestimonialdata");
			...
			if ($_FILES['fileToUpload']['error'] === UPLOAD_ERR_OK) {
				...
				if (!in_array($imageFileType, $allowedExtensions)) {
			        ...
			        header("location: ../client_side_pages.php?error=error=wrongimagetype");
			            ...
			            $check = "SELECT image_url FROM testimonials WHERE test_id='$id';";
			            $check2 = mysqli_query($conn, $check);
			             ...
			             $query = "UPDATE testimonials SET testimonial_name = '$testimonial_name', company = '$testimonial_company', testimony = '$testimony', image_url = '$newFileName' WHERE test_id='$id';";
			            $query2 = mysqli_query($conn, $query);
			   			
			   			if ($query2) {
			   				header("location: ../client_side_pages.php?error=testupdateok");
			   			}
				...
				$query = "UPDATE testimonials SET testimonial_name = '$testimonial_name', company = '$testimonial_company', testimony = '$testimony' WHERE test_id='$id';";
				$query2 = mysqli_query($conn, $query);
				header("location: ../client_side_pages.php?error=testupdateok");
			}
